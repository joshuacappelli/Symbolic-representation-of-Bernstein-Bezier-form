...
def bb1mult(b1, d1, b2, d2, lo=None, hi=None):
    ...
    dg1 -- degree of polynomial 1
    b2 -- array of coefficients of polynomial 2
    dg2 -- degree of polynomial 2
    ...
    if lo is None or hi is None:
        for i1 in range(d1 + 1):
            for i2 in range(d2 + 1):
                mult = comb(d1, i1) * comb(d2, i2) / comb(d1 + d2, i1 + i2)
...
if test == 1:
    dg1 = 2
    dg2 = 1
    b1 = [sympy.symbols('b1_%d' % i) for i in range(dg1+1)]
    b2 = [sympy.symbols('b2_%d' % i) for i in range(dg2+1)]
